...
export const Users: React.FC = () => {
  ...
  useEffect(() => {
    if (departments.length > 0 && (currentUser?.role === 'TRƯỞNG PHÒNG' || currentUser?.role === 'TRƯỞNG NHÓM') && currentUser.department && !formData.department) {
  ...
  const processedUsers = useMemo(() => {
    ...
    let accessibleUsers = allUsers.filter(u => {
      if (currentUser?.role === 'QUẢN TRỊ VIÊN') return true;
    ...
    try {
      if (editingUserId) {
        ...
        const newUserPayload = {
          ...formData,
          supervisorId: (currentUser?.role === 'TRƯỞNG PHÒNG' || currentUser?.role === 'TRƯỞNG NHÓM') && formData.role === 'NHÂN VIÊN' ? currentUser.id : undefined
        };
                    ...
                    <table className="w-full text-left">
                      ...
                      <tbody className="divide-y divide-slate-50">
                        {users.map(u => {
                          ...
                          const canEdit = currentUser?.role === 'QUẢN TRỊ VIÊN' || isMe;
                          const canDelete = currentUser?.role === 'QUẢN TRỊ VIÊN' && !isMe;
                          // Tìm tên Supervisor
                          ...
                          return (
                            <tr key={uid} className="hover:bg-indigo-50/40 transition-colors group">
                              ...
                              <td className="px-6 py-4">
                                <span className={`px-3 py-1 rounded-full text-[11px] font-bold border ${u.role === 'QUẢN TRỊ VIÊN' ? 'bg-purple-100 text-purple-700 border-purple-200' :
                                  u.role === 'TRƯỞNG PHÒNG' ? 'bg-blue-100 text-blue-700 border-blue-200' :
                                    u.role === 'TRƯỞNG NHÓM' ? 'bg-green-100 text-green-700 border-green-200' : 'bg-slate-100 text-slate-600 border-slate-200'
                                  }`}>
                                  {u.role}
                                </span>
                              </td>
              ...
              <div className="grid grid-cols-2 gap-4">
                <div>
                  ...
                  <select className="w-full p-2 border border-slate-200 rounded outline-none mt-1 text-sm"
                    value={formData.role} disabled={currentUser?.role !== 'QUẢN TRỊ VIÊN'} onChange={e => setFormData({ ...formData, role: e.target.value as UserRole })}>
                    {currentUser?.role === 'QUẢN TRỊ VIÊN' && <option value="QUẢN TRỊ VIÊN">Quản trị viên</option>}
                    {currentUser?.role === 'QUẢN TRỊ VIÊN' && <option value="TRƯỞNG PHÒNG">Trưởng phòng</option>}
                    {currentUser?.role === 'QUẢN TRỊ VIÊN' && <option value="TRƯỞNG NHÓM">Trưởng nhóm</option>}
                ...
                <div>
                  ...
                  <select required className="w-full p-2 border border-slate-200 rounded outline-none disabled:bg-slate-100 mt-1 text-sm"
                    disabled={currentUser?.role !== 'QUẢN TRỊ VIÊN' || departments.length === 0}
